...
export async function followDiscovery(directory, action, cohort, extra) {
  ...
  if (action === 'retrieve') {
    ...
    await save(join(directory, `retrieval-${cohort}.json`), results);
    return results;
  }
  ...
  if (action === 'contrast') {
    ...
    if (corpus.source_windows.some(w => w.event_refs.some(id => donorEvents.has(id)))) throw Error('Challenge overlaps the discovery evidence. Assign a separate episode first.');
    ...
    config.request.prompt = `Challenge the assigned definitions against this separate episode. Inspect applicability, competing explanations and disconfirming evidence. Save one discovery investigation with the actual references; retain missing evidence and disagreement. This run has up to ${config.run_budget.max_calls} calls within the shared campaign allowance.`;
  ...
  } else throw Error('Use contrast, extract or retrieve.');
...
export async function runStudy(directory, planPath) {
  ...
  const reportPath = join(directory, `${plan.objective}-study.json`);
  ...
  if (!candidate?.body.instruction_contract?.discovery_refs?.length) throw Error('Select a saved extracted instruction linked to its investigation.');
  for (const reference of candidate.body.instruction_contract.discovery_refs) {
    const investigation = records.find(r => r.id === reference.id && r.version === reference.version && r.kind === 'discovery' && r.body.decision === 'supported' && r.body.occurrence_refs.length);
    if (!investigation) throw Error('Candidate investigation is absent or incomplete.');
  }
  if (plan.objective === 'system_benefit') {
    const prior = await read(join(directory, 'function-study.json'));
