...
export async function run(directory) {
  ...
  try {
    ...
    await writeFile(join(directory, 'records.json'), json([{ kind: 'experiment', provenance, body: { name: 'installed-functional-reuse', template: 'transfer', study_objective: 'function', candidate: report.candidate, baseline: { id: baseline.id, version: baseline.body.version }, hypothesis: 'The discovered policy fulfills the recipient contract including unresolved input', case_ids: config.policies[0].case_ids, scenario_families: ['recipient-reconciliation'], feedback: 'aggregate', model_version: config.request.model, tool_versions: [], memory_start_refs: [], repetitions: 1, budget: config.grant.budget, metrics: ['quality'], policy_id: config.policies[0].id, selection_frozen: true } }]));
