...
Registered-tool identity includes its configuration and the size and modification time of its executable and absolute file arguments. Use optional `code_files` (at most 32 absolute paths) for additional checker code or configuration dependencies. These checks use filesystem metadata and do not read entire executables. An ordinary edit or replacement makes earlier validation ineligible for transfer; changes that preserve both size and modification time require the owner to rerun checks. Relative imports, inherited environment and remote state must be declared when relevant. Inputs in `reads` and targets in `validates` remain separate.
