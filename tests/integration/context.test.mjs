...
for (const mode of ['resume', 'active']) for (const withdrawal of ['retirement', 'delete', 'expiry', 'access']) test(`R1-01/02: ${mode} context excludes ${withdrawal} memory and its interpretation at the provider boundary`, async () => {
  ...
  try {
    ...
    config.request.prompt = JSON.stringify({ directory, memory, mode, withdrawal });
    await writeFile(configFile, JSON.stringify(config));
    ...
    if (mode === 'resume') {
      ...
      retirement.request.prompt = JSON.stringify({ directory, memory, mode: 'retire', withdrawal });
...
test('R1-06: more than ten bridge frames of retained visible tool results resume through a bounded descriptor and protocol tail', async () => {
  ...
  try {
    // The stress case measures retained history and resume, with 210 reads and
    // their compactions. Leave room for slower CI hosts; deadlines have a separate test.
    const runAllowanceMs = 300000;
    const processTimeoutMs = runAllowanceMs + 30000;
    ...
    const config = {
      ...
      grant: { id: 'long-grant', scope: { client: 'long-client', project: 'long-project' }, mode: 'observe', paths: ['report.txt'], tools: [], profiles: ['caretaker'],
        budget: { max_calls: 500, max_tokens: '2000000', max_cost_microusd: '1000000', max_actions: 5, max_work_items: 5, max_depth: 2, deadline_ms: String(Date.now() + runAllowanceMs) }, context: 'long-context', visible_splits: ['development'], allow_export: false },
    ...
    await writeFile(configFile, JSON.stringify(config));
    const first = command(['run', configFile], processTimeoutMs); assert.equal(first.status, 2, first.stderr + first.stdout);
    assert.equal(JSON.parse(first.stdout).disposition, 'interrupted', first.stdout);
    ...
    const resumed = command(['run', configFile], processTimeoutMs); assert.equal(resumed.status, 0, resumed.stderr + resumed.stdout);
